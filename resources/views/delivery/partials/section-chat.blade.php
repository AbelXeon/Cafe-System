    ...
    <div class="flex h-full w-full overflow-hidden">
        ...
        <div class="w-full md:w-80 border-r border-[#1e1c25] flex-col shrink-0 h-full overflow-hidden"
            ...
            <div class="p-4 sm:p-5 border-b border-[#1e1c25] shrink-0 bg-[#0f0e13]">
                <h1 class="text-lg sm:text-xl font-bold text-white tracking-tight">Driver Chat</h1>
                <p class="text-stone-500 text-xs mt-0.5">Talk to customers while delivering their orders</p>
            </div>
            ...
            <div class="flex-1 overflow-y-auto custom-scroll divide-y divide-[#1e1c25]/40">
                    ...
                    <button @click="openConversation(conv)"
                        ...
                        <div class="flex items-center justify-between gap-2">
                            ...
                            <span class="w-2.5 h-2.5 rounded-full shrink-0" 
                                  ...
                                  :title="conv.can_chat ? 'Active delivery' : 'Chat closed'"></span>
                        </div>
                ...
                <div x-show="conversations.length === 0" class="p-8 text-center text-stone-500 text-xs flex flex-col items-center">
                    ...
                    <span>No active or past customer chats.</span>
                    <span class="mt-1 text-[11px] text-stone-600">Chats open automatically when you accept an incoming delivery.</span>
                </div>
            </div>
        </div>
        ...
        <div class="flex-1 flex-col min-h-0 h-full overflow-hidden bg-[#0f0e13]/60"
            ...
            <!-- Blank state when no conversation is selected on PC -->
            <div x-show="!activeOrderId" class="flex-1 flex flex-col items-center justify-center text-center p-6">
                ...
                <p class="text-stone-500 text-xs mt-1 max-w-xs">Click any order on the left to start real-time messaging with your customer.</p>
            </div>

            <!-- Active Chat Thread -->
            <div x-show="activeOrderId" class="flex-1 flex flex-col min-h-0 h-full">
                
                <!-- Thread Header -->
                <div class="p-3.5 sm:p-4 border-b border-[#1e1c25] bg-[#0f0e13] flex items-center justify-between gap-3 shrink-0">
                    <div class="flex items-center gap-3 min-w-0">
                        <button @click="backToList()" class="md:hidden p-2 -ml-1 rounded-xl text-stone-400 hover:text-white hover:bg-[#1e1c25] transition cursor-pointer">
                        ...
                        <div class="min-w-0">
                            ...
                            <span class="text-[11px] text-stone-400 block" x-text="canSend ? 'Live Delivery in Progress' : 'Delivery Completed (Closed)'"></span>
                        </div>
                    </div>
                    ...
                    <span class="text-[10px] uppercase font-extrabold px-2.5 py-1 rounded-full shrink-0 flex items-center gap-1.5"
                        ...
                        <span x-text="canSend ? 'Active Delivery' : 'Closed'"></span>
                    </span>
                </div>
                ...
                <div x-ref="messageList" class="flex-1 overflow-y-auto custom-scroll p-4 sm:p-6 space-y-3.5">
                    ...
                    <div x-show="!loading && messages.length === 0" class="text-center text-stone-500 text-xs py-12 flex flex-col items-center">
                        ...
                        <span>No messages yet. Send an update to the customer!</span>
                    </div>
                </div>
                ...
                <div class="p-3 sm:p-4 border-t border-[#1e1c25] bg-[#0f0e13] shrink-0">
                    <div x-show="!canSend" class="text-xs text-stone-500 text-center py-2 bg-[#14131a] rounded-xl border border-[#2a2731]">
                        This chat is closed. Messaging is only enabled while the order is out for delivery.
                    </div>
                    
                    <form x-show="canSend" @submit.prevent="send()" class="flex items-center gap-2">
                        <input x-model="draft" type="text" placeholder="Type a message to the customer..."
                        ...
                        <button type="submit" :disabled="!draft.trim()"
                            class="w-11 h-11 rounded-xl bg-[#b08d57] hover:bg-[#c9a36b] disabled:opacity-40 text-[#0f0e13] font-bold flex items-center justify-center transition shrink-0 shadow-lg shadow-[#b08d57]/20 active:scale-95 cursor-pointer">
