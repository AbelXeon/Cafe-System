<!-- SECTION: Admin Profile & Account Settings -->
<div id="section-profile" class="page-section hidden space-y-6 sm:space-y-8 max-w-4xl pb-20" x-data="adminProfileApp()" x-init="init()">

    <!-- Page Header -->
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">Admin Account Settings</h1>
            <p class="text-stone-400 text-xs sm:text-sm mt-1">Manage your administrative credentials, personal details, and master security</p>
        </div>
        <div class="flex items-center gap-2 self-start sm:self-auto bg-[#14131a] border border-[#2a2731] rounded-xl px-3 py-1.5 shadow-inner">
            ...
            <span class="text-xs font-semibold text-stone-300">System Administrator</span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3.5 sm:gap-4">
        <div class="bg-[#14131a] border border-[#2a2731] rounded-2xl p-4 sm:p-5 flex items-center gap-4 shadow-lg">
            ...
            <div>
                <span class="text-xs text-stone-500 font-medium block">Total Platform Orders</span>
                <span class="text-lg sm:text-xl font-black text-white">{{ number_format($totalOrders ?? 0) }}</span>
            </div>
        </div>
        ...
        <div class="bg-[#14131a] border border-[#2a2731] rounded-2xl p-4 sm:p-5 flex items-center gap-4 shadow-lg">
            ...
            <div>
                <span class="text-xs text-stone-500 font-medium block">Staff & Drivers</span>
                <span class="text-lg sm:text-xl font-black text-white">{{ count($staff ?? []) }}</span>
            </div>
        </div>
        ...
        <div class="bg-[#14131a] border border-[#2a2731] rounded-2xl p-4 sm:p-5 flex items-center gap-4 shadow-lg">
            ...
            <div>
                <span class="text-xs text-stone-500 font-medium block">Admin Since</span>
        ...
        <div class="flex items-center justify-between pb-5 border-b border-[#1e1c25]">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-stone-500">Update your administrator name, email, and phone contact</p>
                </div>
            </div>
        ...
        <form x-show="editingProfile" x-cloak @submit.prevent="saveProfile()" class="mt-6 space-y-4 sm:space-y-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        <i data-lucide="phone" class="w-4 h-4 text-stone-500 absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none"></i>
                        <input x-model="profileForm.phone" type="tel" placeholder="e.g. +251 900 000 000"
                            class="cd-input w-full bg-[#0f0e13] border border-[#2a2731] rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-stone-600 focus:outline-none transition">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    <i data-lucide="mail" class="w-4 h-4 text-stone-500 absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none"></i>
                    <input x-model="profileForm.email" type="email" required placeholder="admin@domain.com"
                        class="cd-input w-full bg-[#0f0e13] border border-[#2a2731] rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-stone-600 focus:outline-none transition">
                </div>
            </div>
        ...
        <div class="flex items-center justify-between pb-5 border-b border-[#1e1c25]">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-stone-500">Ensure your administrative account uses a strong password</p>
                </div>
            </div>
        </div>
        ...
        <form @submit.prevent="updatePassword()" x-data="{ showCurrent: false, showNew: false, showConfirm: false }" class="mt-6 space-y-4">
            ...
            <div class="flex justify-end pt-2">
                <button type="submit" :disabled="savingPassword"
                    class="bg-[#1e1c25] hover:bg-[#2a2731] border border-[#2a2731] hover:border-[#b08d57]/40 text-stone-200 font-bold text-xs sm:text-sm px-5 py-2.5 rounded-xl transition flex items-center gap-2 cursor-pointer active:scale-95 disabled:opacity-50">
    ...
    <div class="bg-[#14131a] border border-rose-900/30 rounded-2xl p-5 sm:p-7 shadow-xl">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-stone-500 mt-0.5">End your administrative session securely</p>
            </div>
