...
<main x-show="currentView === 'orders'" class="flex-1 flex flex-col min-h-0 bg-[#14131a]/40 w-full overflow-hidden">
    <!-- Header Bar -->
    <div class="bg-[#0f0e13]/98 backdrop-blur-xl border-b border-[#2a2731] px-4 sm:px-6 lg:px-8 py-3.5 shrink-0 flex flex-wrap items-center justify-between gap-4">
        <div>
        ...
        <div class="flex items-center gap-2 overflow-x-auto custom-scroll">
            <button @click="activeTab = 'all'"
                ...
                class="px-3 py-1.5 rounded-xl text-xs font-semibold transition shrink-0 cursor-pointer">
            ...
            <button @click="activeTab = 'pending'"
                ...
                class="px-3 py-1.5 rounded-xl text-xs font-semibold transition shrink-0 flex items-center gap-1.5 cursor-pointer">
    ...
    <!-- Orders Grid Stream (Displays 3 columns horizontally with items-start to prevent blank empty areas) -->
    <div class="flex-1 overflow-y-auto custom-scroll p-3.5 sm:p-5 lg:p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-4 items-start">
            <template x-for="order in filteredOrders" :key="order.id">
                <div class="bg-[#14131a] border rounded-2xl overflow-hidden flex flex-col shadow-xl transition-all duration-200 min-w-0"
                     :class="{
                        ...
                        'border-[#2a2731] opacity-75': order.status === 'completed' || order.status === 'cancelled'
                     }">
                    
                    <!-- Order Ticket Header -->
                    <div class="p-3.5 sm:p-4 border-b border-[#2a2731] bg-[#0f0e13]/60 flex items-start justify-between gap-3 min-w-0">
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2 flex-wrap">
                                <span class="text-sm sm:text-base font-black text-white shrink-0" x-text="'Order #' + order.id"></span>
                            ...
                            <p class="text-xs text-stone-400 mt-1 flex items-center gap-1.5 flex-wrap min-w-0">
                    ...
                    <!-- Order Image Preview Display (Compact size for 3-card layout) -->
                    <template x-if="getOrderImage(order)">
                        <div class="w-full h-32 bg-[#0f0e13] border-b border-[#2a2731] overflow-hidden relative group">
                    ...
                    <div x-show="order.special_note" class="px-3.5 py-2.5 bg-amber-500/10 border-b border-amber-500/30 text-xs min-w-0">
                        <div class="flex items-center gap-1.5 font-extrabold uppercase text-[10px] text-amber-400 tracking-wider mb-1">
                            <i data-lucide="alert-triangle" class="w-3 h-3 text-amber-400 shrink-0"></i>
                            <span>Special Order Note:</span>
                        </div>
                        <div class="bg-[#0f0e13]/90 text-white font-medium p-2 rounded-lg border border-amber-500/25 leading-relaxed whitespace-pre-line break-all sm:break-words min-w-0 text-xs" x-text="order.special_note"></div>
                    </div>

                    <!-- Order Itemized List (Wraps tightly to prevent empty gap below items) -->
                    <div class="p-3 sm:p-3.5 space-y-2.5 max-h-80 overflow-y-auto custom-scroll min-w-0">
                        <template x-for="item in order.items" :key="item.id">
                            <div class="bg-[#0f0e13] border border-[#2a2731] rounded-xl p-3 flex flex-col gap-2 min-w-0 w-full overflow-hidden">
                                <div class="flex items-start justify-between gap-2.5 min-w-0">
                                    <div class="flex items-center gap-2 min-w-0 flex-1">
                                        <span class="w-5 h-5 rounded-md bg-[#b08d57] text-[#0f0e13] font-black text-[11px] flex items-center justify-center shrink-0" x-text="item.quantity + 'x'"></span>
                                        <span class="text-xs sm:text-sm font-bold text-white truncate min-w-0" x-text="item.name"></span>
                                    </div>
                                ...
                                <div x-data="{ details: getItemDetails(item) }" 
                                     ...
                                     class="pt-2 border-t border-[#1e1c25] space-y-2 min-w-0 w-full overflow-hidden">
                                    
                                    <!-- Extras & Add-ons Section -->
                                    <template x-if="details.extras && details.extras.length > 0">
                                        <div class="space-y-1 min-w-0 w-full">
                                            <div class="flex items-center gap-1 font-black uppercase text-[10px] text-emerald-400 tracking-wider">
                                                <i data-lucide="plus-circle" class="w-3 h-3 text-emerald-400 shrink-0"></i>
                                                <span>Extras & Add-ons</span>
                                            </div>
                                            <div class="space-y-1 min-w-0 w-full">
                                                <template x-for="(extra, idx) in details.extras" :key="idx">
                                                    <div class="flex items-start gap-1.5 text-xs font-medium text-white bg-[#191722] border border-[#2a2731] px-2 py-1 rounded-md shadow-sm min-w-0 w-full overflow-hidden">
                                    ...
                                    <template x-if="details.notes && details.notes.length > 0">
                                        <div class="space-y-1 min-w-0 w-full">
                                            <div class="flex items-center gap-1 font-black uppercase text-[10px] text-amber-400 tracking-wider">
                                                <i data-lucide="file-text" class="w-3 h-3 text-amber-400 shrink-0"></i>
                                                <span>Special Instructions</span>
                                            </div>
                                            <div class="space-y-1 min-w-0 w-full">
                                                <template x-for="(noteLine, nIdx) in details.notes" :key="nIdx">
                                                    <div class="flex items-start gap-1.5 text-xs font-medium text-white bg-amber-500/10 border border-amber-500/30 px-2 py-1 rounded-md leading-relaxed shadow-sm min-w-0 w-full overflow-hidden">
                                                        <i data-lucide="corner-down-right" class="w-3 h-3 text-amber-400 shrink-0 mt-0.5"></i>
                                    ...
                                    <template x-if="!details.hasContent && item.special_note">
                                        <div class="bg-amber-500/10 border border-amber-500/30 rounded-lg p-2 min-w-0 w-full overflow-hidden">
                                            <div class="flex items-center gap-1 font-bold uppercase text-[10px] text-amber-400 tracking-wider mb-1">
                                                <i data-lucide="file-text" class="w-3 h-3 text-amber-400 shrink-0"></i>
                                                <span>Item Note</span>
                                            </div>
                                            <p class="text-xs font-medium text-white bg-[#0f0e13]/80 p-1.5 rounded-md border border-amber-500/20 leading-relaxed whitespace-pre-line break-all sm:break-words min-w-0" x-text="item.special_note"></p>
                                        </div>
                                    </template>
                                </div>
                            </div>
                    ...
                    <div class="p-3 sm:p-3.5 border-t border-[#2a2731] bg-[#0f0e13]/70 flex items-center gap-2 mt-auto">
                        
                        <!-- If Pending: Accept / Start Preparing Button -->
                        <template x-if="order.status === 'pending'">
                            <button @click="updateStatus(order, 'preparing')"
                                    :disabled="updatingId === order.id"
                                    class="w-full bg-amber-500 hover:bg-amber-400 text-black font-extrabold text-xs py-2.5 rounded-xl transition flex items-center justify-center gap-2 shadow-lg shadow-amber-500/10 cursor-pointer disabled:opacity-50">
                        ...
                        <template x-if="order.status === 'preparing'">
                            <div class="w-full flex gap-2">
                                <button @click="updateStatus(order, 'ready')"
                                        :disabled="updatingId === order.id"
                                        class="flex-1 bg-emerald-500 hover:bg-emerald-400 text-black font-extrabold text-xs py-2.5 rounded-xl transition flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/10 cursor-pointer disabled:opacity-50">
                                    <i data-lucide="check" class="w-4 h-4 stroke-[3]"></i>
                                    <span>Mark Food as Done</span>
                                </button>
                            </div>
                        </template>
                        ...
                        <template x-if="order.status === 'ready'">
                            <button @click="updateStatus(order, 'completed')"
                                    :disabled="updatingId === order.id"
                                    class="w-full bg-[#1e1c25] hover:bg-[#2a2731] text-stone-200 font-bold text-xs py-2.5 rounded-xl transition flex items-center justify-center gap-2 border border-[#2a2731] cursor-pointer disabled:opacity-50">
                        ...
                        <template x-if="order.status === 'completed' || order.status === 'cancelled'">
                            <div class="w-full py-1.5 text-center text-xs text-stone-500 font-semibold">
                                Ticket Closed
                            </div>
                        </template>
                    </div>
